...
class TestGeocodeAddress:
    async def test_success_returns_geopoint_with_correct_coordinates(
        ...
        async with httpx.AsyncClient() as client:
            result = await geocode_address(
                client, "157 bd MacDonald 75019 Paris",
                geocoding_url=GEOCODING_URL, score_threshold=SCORE_THRESHOLD,
            )
    ...
    async def test_score_exactly_at_threshold_is_accepted(
        ...
        async with httpx.AsyncClient() as client:
            result = await geocode_address(
                client, "some address",
                geocoding_url=GEOCODING_URL, score_threshold=0.4,
            )
        assert isinstance(result, GeoPoint)
    ...
    async def test_503_raises_geocoding_service_error(
        self, respx_mock: respx.MockRouter
    ) -> None:
    ...
    async def test_429_raises_geocoding_service_error(
        self, respx_mock: respx.MockRouter
    ) -> None:
    ...
    async def test_timeout_propagates_as_httpx_timeout_exception(
        ...
        respx_mock.get(GEOCODING_URL).mock(
...
class TestGeocodeBatch:
    ...
    async def test_result_keys_match_input_keys(
        self, respx_mock: respx.MockRouter
    ) -> None:
        respx_mock.get(GEOCODING_URL).mock(return_value=_ok())
        results = await geocode_batch(
    ...
    async def test_all_fail_returns_all_exceptions(
        self, respx_mock: respx.MockRouter
    ) -> None:
        respx_mock.get(GEOCODING_URL).mock(return_value=_empty())
        results = await geocode_batch(
